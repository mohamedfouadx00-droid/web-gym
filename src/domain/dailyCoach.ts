...
export function normalizePreferences(raw: UserPreferences | undefined, userId: string): UserPreferences {
  ...
  return {
    ...
    maxNotificationsPerDay: Math.min(24, Math.max(1, Number(source?.maxNotificationsPerDay) || 12)),
    ...
    gymNotificationsEnabled: source?.gymNotificationsEnabled ?? true,
    sleepAlarmEnabled: source?.sleepAlarmEnabled ?? true,
    sleepAlarmAfterHours: Math.min(12, Math.max(4, Number(source?.sleepAlarmAfterHours) || 8)),
    sleepAlarmSound: ['loud', 'classic', 'bell', 'soft'].includes(String(source?.sleepAlarmSound))
      ? source?.sleepAlarmSound as UserPreferences['sleepAlarmSound']
      : 'loud',
    sleepAlarmVibration: ['off', 'normal', 'strong'].includes(String(source?.sleepAlarmVibration))
      ? source?.sleepAlarmVibration as UserPreferences['sleepAlarmVibration']
      : 'strong',
    sleepAlarmGradualVolume: source?.sleepAlarmGradualVolume ?? true,
    sleepAlarmSnoozeMinutes: [5, 10, 15, 20, 30].includes(Number(source?.sleepAlarmSnoozeMinutes))
      ? Number(source?.sleepAlarmSnoozeMinutes)
      : 10,
    sleepAlarmMaxSnoozes: Math.min(5, Math.max(0, Number(source?.sleepAlarmMaxSnoozes) || 2)),
  }
}
