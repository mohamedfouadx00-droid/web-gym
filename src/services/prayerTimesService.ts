...
export type PrayerLocationPermissionStatus = 'granted' | 'prompt' | 'denied' | 'disabled' | 'unsupported' | 'unavailable'

interface SimplePosition {
  coords: { latitude: number; longitude: number }
}

function mapPermission(value?: string): PrayerLocationPermissionStatus {
  if (value === 'granted') return 'granted'
  if (value === 'denied') return 'denied'
  return 'prompt'
}

export async function getPrayerLocationPermissionStatus(): Promise<PrayerLocationPermissionStatus> {
  try {
    if (Capacitor.isNativePlatform()) {
      const status = await Geolocation.checkPermissions()
      if (status.coarseLocation === 'granted' || status.location === 'granted') return 'granted'
      if (status.coarseLocation === 'denied' && status.location === 'denied') return 'denied'
      return mapPermission(status.coarseLocation ?? status.location)
    }
...
export async function requestPrayerLocationPermission(): Promise<PrayerLocationPermissionStatus> {
  try {
    if (Capacitor.isNativePlatform()) {
      const status = Capacitor.getPlatform() === 'android'
        ? await Geolocation.requestPermissions({ permissions: ['coarseLocation'] })
        : await Geolocation.requestPermissions()
      if (status.coarseLocation === 'granted' || status.location === 'granted') return 'granted'
      return status.coarseLocation === 'denied' && status.location === 'denied' ? 'denied' : 'prompt'
    }
    await getPosition()
    return 'granted'
  } catch (error) {
    const message = error instanceof Error ? error.message.toLowerCase() : ''
    if (message.includes('disabled') || message.includes('location services')) return 'disabled'
    return 'denied'
  }
}

async function getPosition(): Promise<SimplePosition> {
  if (Capacitor.isNativePlatform()) {
    const permission = await getPrayerLocationPermissionStatus()
    if (permission !== 'granted') {
      const requested = await requestPrayerLocationPermission()
      if (requested !== 'granted') throw new Error(`geolocation-${requested}`)
    }
