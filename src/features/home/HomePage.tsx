...
useEffect(() => {
  ...
  const coachContext = useMemo<CoachQuestionContext | undefined>(() => smartSnapshot ? ({
    ...
    frequentFoods,
    preferences,
    nextTaskTitles: tasks.filter((task) => !task.completed).slice(0, 4).map((task) => task.title),
  }) : undefined, [smartSnapshot, availableFoods, frequentFoods, preferences, tasks])
  ...
  async function openCoach() {
    ...
    await coachMessageRepo.add({
      ...
      text: 'أنا شايف بيانات يومك الحالية. قولّي حصل إيه أو اسألني عن الأكل أو الجيم أو المياه أو الكرياتين.',
    })
  }
...
{showCoach && (
  <div className="modal-backdrop" onClick={() => setShowCoach(false)}>
    <div role="dialog" aria-modal="true" className="modal smart-coach-modal" onClick={(event) => event.stopPropagation()}>
      ...
      <p className="muted">الرد يعتمد على وقتك ونومك وأكلك ومياهك ومكانك وخطة اليوم. المحرك المحلي يعمل دائمًا، والـAI الخارجي اختياري من الإعدادات.</p>
