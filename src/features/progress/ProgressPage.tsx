...
export default function ProgressPage() {
  ...
  const todayScore = todayTasks.length ? Math.round((todayCompleted / todayTasks.length) * 100) : 0
  ...
  return (
    <Page title="تقدمي" subtitle="التقييم بيطلع تلقائيًا من اللي سجلته وعملته، من غير زر «هخلص يومي»">
      <div className="stats-grid">
        ...
        <Stat label="تقييم اليوم التلقائي" value={`${todayScore}%`} hint={`${todayCompleted} من ${todayTasks.length} خطوات`} />
      ...
      <Card title="تنفيذ خطوات اليوم">
        <div className="adherence-row">
          ...
          <div><strong>{todayScore}%</strong><span>محسوب تلقائيًا من اللي سجلته</span></div>
        </div>
        <ProgressBar value={todayCompleted} max={todayTasks.length || 1} />
      </Card>
